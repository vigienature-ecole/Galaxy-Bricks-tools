<tool id="Import_oiseaux_VN" name="Importer les données Opération escargot Vigie-Nature École">
  <description>Importer des données escargots depuis la base de données VN</description>
  <requirements>
    ...
    <requirement type="package" version="1.12.2">r-data.table</requirement>
      ...
      <command detect_errors="exit_code"><![CDATA[
        ...
        <param name="observatoire" type="select" label="Choisissez les variables à importer" multiple="false" display="radio">
          <option value="Oiseaux_des_jardins_zo">Données espèces et données zones d'observation</option>
            <option value="Oiseaux_des_jardins_clc">Données espèces et données occupation du sol</option>
            <option value="Oiseaux_des_jardins_bioclim">Données espèces et données climatiques</option>
        </param>
    </inputs>
    <outputs>
